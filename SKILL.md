...
**搜到什么写什么，搜不到就写"未找到"。** 型号、口径、规格、参数、系列名——没有搜索来源的，一律不能写进 aiRemark、不能推荐给用户、不能作为决策依据。

**具体禁令：**
- ❌ 根据命名规则"拼凑"型号（如"SMC 命名规则是 AW+口径，那 AW80-60 应该存在"）
- ❌ 把 CAD 文件名、网页文件名、URL 片段当成真实产品型号
- ❌ 用训练知识补充搜索结果的空白（如"SMC AW 系列最大口径应该到 2 寸"）
- ❌ 搜不到就"猜一个大概的"写上去

**正确做法：**
- 搜索结果里有的 → 写入，附来源链接
- 搜索结果里没有的 → 写"未找到对应产品/型号"，不编造
- 不确定是否匹配的 → 标 ⚠️，写清差异，不强行下结论

**来源绑定规则：** 每条产品信息必须同时标注来源（官网/B2B平台/搜索引擎/企查查），没有来源 = 不能写。
