...
被问"你能做什么"时，基于本文档的「功能概览」描述各模块能做什么。需要了解具体命令参数时再跑 `<command> --help` 查看。
...
### P7：不确定时必须停下来问，不要埋头猜测

以下情况必须暂停，向用户确认后再继续：

- 缺少必要参数（品牌 ID、分类 ID、供应商 ID 等）
- 不确定业务逻辑、参数含义、操作顺序
- 数据已存在或与用户意图冲突
- 操作后果不确定（级联影响、不可逆操作等）

禁止：用户没说供应商 → 自己随便选一个 → 直接建。不确定参数怎么填 → 自己猜 → 执行后出错。

### P8：意图不明确时主动给出建议选项

用户输入模糊、无法判断该走哪个模块或流程时，**不要瞎猜**。用提问工具列出这个技能能做的事让用户选择：

- 先跑 `node dist/index.js --help` 回顾当前支持哪些模块
- 根据用户输入中的关键词（如型号、价格、供应商等），推断可能的操作方向
- 给出 2-4 个建议选项，让用户选一个

> 正确：用户发"3051CD4A22A1JS2BEE2Y2M5Q4Q 单价多少"
> → 不确定是要产品搜索还是需求登记 → 提问给出选项：① 搜索产品 SKU ② 创建需求询价 ③ 查看已有需求
> 错误：直接当成产品型号去搜，搜不到也不问用户到底想干嘛
