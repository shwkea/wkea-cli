...
### P14：Sub-agent 通用执行原则

涉及多个独立对象或多步骤的长流程（需求处理、批量开发供应商等），按本节规则开 sub-agent 并行执行。**主 agent 不亲自跑流程，只做调度。**

#### 何时开 sub-agent

满足以下任一条件：

- 处理对象 ≥ 2 个相互独立的实体（多行项目、多品牌、多 SKU）
- 单实体处理步骤 ≥ 5 步且涉及大量搜索/验证
- 流程中可能出现长时间阻塞（二次确认、等待供应商回复）

**不满足则主 agent 直接跑**，不要为了拆而拆。

#### Sub-agent 必须做

| 规则 | 说明 |
|---|---|
| **状态写回系统** | 每完成一小步立即 `progress step`，不只在内存里 |
| **边做边写 aiRemark** | 每搜一次写一次，不攒到最后 |
| **幂等** | 重跑同一段不会产生副作用（查重、读已写内容再追加） |
| **心跳** | 每 2-3 分钟更新一次 `progress lastRunAt` |
| **必读模板** | 写 aiRemark 前先读 `docs/modules/demand-aiRemark-template.md` |
...
| 规则 | 说明 |
|---|---|
| **传参完整** | 启动 sub-agent 时传：任务类型、业务 ID、进度 ID、起始步骤、重试上下文 |
| **不读 sub-agent 的中间产物** | 不读搜索结果、不读 aiRemark 全文，只读 `progress get` 状态 |
| **重试 ≤ 3 次** | 同一 sub-agent 失败 3 次后跳过该对象，继续其他 |
| **异常不阻塞** | 二次确认走软通知 + 异步等回复，不硬阻塞其他 sub-agent |
| **依赖关系自己控** | 供应商开发等必须等产品研究完成，不让 sub-agent 互相等 |

#### 并发限制

| 项目 | 默认并发 | 说明 |
|---|---|---|
| 单次 sub-agent 任务（产品研究、供应商开发等） | **3** | 同时最多 3 个，超出的排队等待 |
| 单次重试 | 1 | 失败后等结果回来再重试，不并发重试 |

**为什么是 3：**
- 大多数 AI 工具 sub-agent 上限是 16/cpu 核，3 是安全范围
- 同时跑太多会触发系统限流/超时，得不偿失
- 用户对单需求处理的可等待时长有限（建议 < 10 分钟）

**行项目数 > 3 时：** 主 agent 维护一个队列，每批 3 个并行，跑完一批再起下一批。

#### 演示/测试模式

测试/演示场景（无 web 搜索权限、用户明确指示、或纯流程验证）：

- **主 agent 可直接执行 `progress step`**（summary 加 `[DEMO]` 前缀），跳过 sub-agent
- **不写 aiRemark**（避免污染真实数据）
- **不创建产品/供应商/询价**（避免污染业务数据）
- **适用：** 单元测试、流程演示、新员工培训

**不要用演示模式做真实业务**——会绕开 P13（防编造）和搜索验证，导致 aiRemark 为空。

#### 进度异常场景处理

| 场景 | 表现 | 处理 |
|---|---|---|
| 进度已 status=done | `progress step` API 拒绝 | 不重建进度，sub-agent 继续把研究结果写入 aiRemark。主 agent 用单独笔记记录"已完成进度需要重跑阶段 N"，下次会话先看 aiRemark 状态再决定 |
| 进度某 step 卡住 | 状态=deadlock 或多次重试失败 | 标记该 step `--abnormal`，主 agent 暂停该阶段，跳过/换批次，等客户或人工介入 |
| sub-agent 写到一半主 agent 挂了 | 进度 step 部分已写 | sub-agent 写的 step 留着，主 agent 恢复时先 `progress get` 拉真实状态再决策 |
| 进度 step 与实际不符 | 比如 step 显示 done 但 aiRemark 是空的 | 以 aiRemark 为准，主 agent 检测到这种状态就重新标 step 异常 |

#### 失败处理

| 失败模式 | 检测 | 处理 |
|---|---|---|
| 挂了 | exit code ≠ 0 | 重派 sub-agent，附 lastError |
| 出错 | 返回 status=error | 重派，附错误信息 |
| 卡住 | 10 分钟无心跳 | 先 `progress get` 看状态，再决定重派或杀掉 |
| 二次确认 | 返回 status=abnormal | 推送给客户等回复，**不重试 sub-agent** |
