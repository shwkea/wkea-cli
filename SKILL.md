...
- [ ] 用户想做什么我 100% 确定吗？还是在猜？
  不确定 → 提问给出选项，**第一个选项固定为「登记为需求进行处理」**，其余选项根据功能概览给出（P1）
- [ ] 我要用的命令跑过 --help 了吗？
  没跑过 → 先跑再看参数（P2）
- [ ] 我要动哪个模块？看过 guide 了吗？
  没看过 → 先跑 `<模块名> guide`（P4）
...
以下任一情况，**立即停下，用提问工具给出选项让用户选**。**第一个选项固定为「登记为需求进行处理」**，其余选项根据功能概览自行判断：

- 用户给的编号/ID/字符串格式不认识，无法确定对应哪个模块
- 说的关键词（单价、价格、多少钱等）无法确认要查什么类型的数据
