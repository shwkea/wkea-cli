...
- ✅ 只说"拉到了哪几条提交，每条改成什么"
- ✅ 用大白话（参照 AGENTS.md 中 Commit 规范）
- ❌ 禁止说 git pull / npm install 这类过程细节
...
### P5：外部数据必须先映射到系统字段再操作

从 PDF、网页、表格等外部来源拿到数据后，**禁止**直接透传原始字段名和值。必须逐字段做映射：

1. **逐字段判断**：这个字段对应系统哪个参数？查 `--help` 确认
2. **名称→ID**：所有名称类字段（品牌、供应商、分类）必须先 `list --name <名>` 查询拿到 ID，查不到就提醒"XX 不存在，是否先创建？"
3. **系统没有的字段** → 放 `--extra-columns`（见 P6）
4. **规格建模**：型号中被分隔符隔开的**每一段**都必须对应一个规格（固定或可变），不能跳过任何一段
5. **SKU 数量按产品类型定**：
   - **有规格选型**（多规格用户可选）→ 只建 **1 个代表 SKU**（model=代表完整型号），电商端选型生成实际型号。**禁止为每个组合建 SKU**
   - **无规格**（单一型号）→ 每个型号建 1 个 SKU

> 正确：PDF 写 `flowRate: 10.5 gpm` → 系统无此字段 → 放 `--extra-columns '{"flowRate":"10.5 gpm"}'`
> 错误：PDF 写 `flowRate: 10.5 gpm` → 直接当系统字段 `--flow-rate 10.5`（这个参数不存在）
> 错误：有规格产品（4×3×7×2×2=336 组合）→ 建 336 个 SKU（应为 1 个代表 SKU）

### P6：系统没有对应字段的信息用附加列保存
