...
export function registerSalesContractCommands(program: Command) {
  ...
  program
    ...
    .requiredOption('--distribution-mode <number>', '配送方式（118=整单发货 119=有货先发 403=堂食，enum --type 配送方式 查看可用值）')
    .requiredOption('--pay-type <number>', '支付方式（枚举ID: 支付方式，enum --type 支付方式 查看可用值）')
