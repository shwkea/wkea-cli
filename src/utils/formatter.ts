...
export function formatFieldsDoc(
  fields: Array<{ field: string; type: string; desc: string }>
): string {
...
export function formatList<T extends Record<string, unknown>>(
  ...
  if (pagination) {
    parts.push(
      `**总数：${pagination.total}** | 第 ${pagination.current}/${Math.ceil(pagination.total / pagination.size)} 页`
    );
    parts.push('');
  }
  parts.push('```json');
