...
```
┌─────────────────────────────────────────────────────────────┐
│  SKILL.md（启动入口 + 顶层规则 P0-P14 + 更新流程）          │
│  - AI 加载 wkea skill 后先读此文件                            │
│  - P0-P14 是绝对铁律，所有 expert 无条件遵守                  │
...
| 层 | 能做什么 | **不能做什么** |
|---|---|---|
| **SKILL.md** | 定义 P0-P14 顶层规则、CLI 通用用法、枚举速查、更新流程 | 不写业务概念（那是 agent 的事）|
...
**SKILL.md 定义**：
- P0-P14 执行原则（写前必查、写后必验、跳转链接、删除确认...）
