...
**2. 创建进度**
根据获取到的信息（需求ID、需求主题）创建进度，每个任务名带文档编号方便对照：
```bash
wkea-manage-cli progress create \
  ...
  --tasks '[
    ...
    {"name":"2.4 缩短关键词SPU搜索","summary":"缩短关键词再搜SPU"},
    {"name":"2.5 缩短型号SKU搜索","summary":"缩短型号再搜SKU"},
    {"name":"2.6 网上搜索产品","summary":"通过WebSearch搜索产品"},
    {"name":"2.7 验证并绑定SKU","summary":"验证匹配并绑定SKU"},
...
进度创建时会一次性建好 16 个任务。后续每完成一个任务（不论大小），都要立即更新进度：
...
> **注意**：有 SKU 匹配的尽量绑定后再往下走。实在没有匹配也不要停，按品牌名继续供应商匹配，供应商报价确认产品后再创建 SKU。
