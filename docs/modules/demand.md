...
**确认标准：**
- **同品牌 + 同型号完全一致** → 直接绑定已有 SKU
- **客户没指定品牌，搜到 WKEA 品牌的同型号** → 绑定
- **客户没指定品牌，搜到的是其他品牌（如 Festo、SMC）** → **不绑定**，只记录到 aiRemark。上架时按客户原文创建，品牌默认 WKEA
- **品牌或型号不完全一致** → 只记录 aiRemark，不绑定
- **完全搜不到** → 如实记录，继续走供应商匹配

> 客户指定了品牌且系统有匹配 → 绑定。客户没指定品牌且匹配到的是其他品牌 → 不绑定（中间商逻辑：出给客户的默认是 WKEA 品牌，内部记溯源信息）。
