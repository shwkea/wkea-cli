...
### P0.7：多步骤任务必须拆解为可见的任务列表

当任务包含 3 个以上步骤、或涉及多个阶段（调研→分析→执行→验证）时，必须先建立任务列表：

1. **动手前先规划**：先把整个任务拆成原子步骤，列出来
2. **每完成一步立即标记**：不让用户猜测当前进度
3. **遇到阻塞时明确标注**：哪一步卡住了、卡在什么原因

简单任务（一个命令就能搞定的事）不需要建列表，但任何需要用户感知进度的复杂工作都必须做。

> 正确：用户说"处理这个需求" → 建列表 [领取→查详情→产品匹配→供应商匹配→询价→汇总]
> 错误：用户说"处理这个需求" → 直接一路 execute 到底，用户完全不知道现在到哪一步了

### P0.8：系统没有对应字段的信息用附加列保存
